...
#------------------------------------------------------ Class methods -------------------------------------------------------
  ...
  def self.get_users_by_activity
    ...
    User.find_by_sql("select users.id, nick, count(user_id) as videos_num from videos, users where videos.user_id = users.id and videos.analyzed=true group by videos.user_id, users.id, nick order by videos_num desc limit 3;")
  end
